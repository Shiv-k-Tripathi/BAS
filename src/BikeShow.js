...
const BikeShow = ({navigation}) => {
  ...
  const get_data = async () => {
    const bikedata = await firestore().collection('bikedetail').get();
    const getbikedata = bikedata.docs.map(docSnap => docSnap.data());
    if (getbikedata.length % 2 == 0) {
      setLength(getbikedata.length);
    } else {
      setLength(getbikedata.length - 1);
      setTimeout(() => {
        setLast(getbikedata[data.length - 1]);
  ...
  useEffect(() => {
    get_data();
    return () => {
      console.log('bikedata from bike show');
    };
  }, [refresh]);
