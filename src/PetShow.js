...
const PetShow = ({navigation}) => {
  ...
  const get_data = async () => {
    ...
    const getpetdata = petdata.docs.map(docSnap => docSnap.data());
    if (getpetdata.length % 2 == 0) {
      setLength(getpetdata.length);
    } else {
      setLength(getpetdata.length - 1);
      setTimeout(() => {
        setLast(getpetdata[data.length - 1]);
  ...
  useEffect(() => {
    get_data();
    return () => {
      console.log('petdata from bike show');
    };
  }, [refresh]);
  ...
  return (
    <View>
      <View style={{borderWidth: 1, margin: 2, flexDirection: 'row'}}>
        ...
        <View style={{marginHorizontal: 10,justifyContent:'center',alignItem:'center'}}>
          ...
          {loading ? (
            ...
            <TouchableOpacity
              ...
              {loadingImg ? (
               ...
               {vishible==length?<Text>No More Data</Text>:<Text>More</Text>}
                </Text>
              ) : (
                <View
                  style={{
