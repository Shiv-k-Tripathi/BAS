...
const Pets = ({navigation, Screen}) => {
  ...
  const OpenCamera = () => {
    launchCamera({quality: 0.9, maxWidth: 600, maxHeight: 600}, data => {
  ...
  const postData = async () => {
    if (
      ...
      try {
        await firestore().collection('petdetail').add({
          name,
          price,
          year,
          disc,
  ...
  return (
    <KeyboardAvoidingView
      ...
      <ScrollView>
        <Text style={{fontSize: 30, margin: 20}}>Pets Detail</Text>
