...
const MobileShow = ({navigation}) => {
  ...
  const get_data = async () => {
    ...
    const getmobiledata = mobiledata.docs.map(docSnap => docSnap.data());
    if (getmobiledata.length % 2 == 0) {
      setLength(getmobiledata.length);
    } else {
      setLength(getmobiledata.length - 1);
      setTimeout(() => {
        setLast(getmobiledata[data.length - 1]);
        console.log('shiv', last);
      }, 300);
    }
    console.log(length);
    setData(getmobiledata);
  };
  useEffect(() => {
    get_data();
    return () => {
      console.log('mobiledata from bike show');
    };
  }, [refresh]);
