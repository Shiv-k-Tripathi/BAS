...
const CarShow = ({navigation}) => {
  ...
  const get_data = async () => {
    const cardata = await firestore().collection('cardetail').get();
    const getcardata = cardata.docs.map(docSnap => docSnap.data());
    if (getcardata.length % 2 == 0) {
      setLength(getcardata.length);
    } else {
      setLength(getcardata.length - 1);
      setTimeout(() => {
        setLast(getcardata[data.length - 1]);
  ...
  useEffect(() => {
    get_data();
    return () => {
      console.log('cardata from bike show');
    };
  }, [refresh]);
