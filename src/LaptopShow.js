...
const LaptopShow = ({navigation}) => {
  ...
  const get_data = async () => {
    const laptopdata = await firestore().collection('laptopdetail').get();
    const getlaptopdata = laptopdata.docs.map(docSnap => docSnap.data());
    if (getlaptopdata.length % 2 == 0) {
      setLength(getlaptopdata.length);
    } else {
      setLength(getlaptopdata.length - 1);
      setTimeout(() => {
        setLast(getlaptopdata[data.length - 1]);
        console.log('shiv', last);
      }, 300);
    }
    console.log(length);
    setData(getlaptopdata);
  };
  useEffect(() => {
    get_data();
    return () => {
      console.log('laptopdata from bike show');
    };
  }, [refresh]);
