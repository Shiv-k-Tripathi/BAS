...
const ElectronicShow = ({navigation}) => {
  ...
  const get_data = async () => {
    ...
    const getelectronicdata = electronicdata.docs.map(docSnap =>
      docSnap.data(),
    );
    if (getelectronicdata.length % 2 == 0) {
      setLength(getelectronicdata.length);
    } else {
      setLength(getelectronicdata.length - 1);
      setTimeout(() => {
        setLast(getelectronicdata[data.length - 1]);
        console.log('shiv', last);
      }, 300);
    }
    console.log(length);
    setData(getelectronicdata);
  };
  useEffect(() => {
    get_data();
    return () => {
      console.log('electronicdata from bike show');
    };
  }, [refresh]);
