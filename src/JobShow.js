...
const JobShow = ({navigation}) => {
  ...
  const get_data = async () => {
    ...
    const getjobdata = jobdata.docs.map(docSnap => docSnap.data());
    if (getjobdata.length % 2 == 0) {
      setLength(getjobdata.length);
    } else {
      setLength(getjobdata.length - 1);
      setTimeout(() => {
        setLast(getjobdata[data.length - 1]);
  ...
  useEffect(() => {
    get_data();
    return () => {
      console.log('jobdata from bike show');
    };
  }, [refresh]);
